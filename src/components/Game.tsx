...
export default function Game() {
  ...
  const intervalRef = React.useRef<number | NodeJS.Timeout | string | null>(null);
  ...
  const clock = useCallback(function () {
  ...
  async function rollDice() {
    ...
    if (gameState === "Playing" || gameState === "New") {
      ...
      let test1: any = [];
      let test2: any = [];
      let allShownHeld: boolean = false;
      ...
      if (allShownHeld) {
        ...
        setDice(oldDice =>
          oldDice.map(die => {
            return die.isHeld ? die : {...die, ...generateNewDice()};
          })
        );
      }
    }
    ...
    <main style={mainContStyle} ref={exportRef}>
      ...
      <div className="mainHeaderCont">
        ...
        <div className="mainHeaderDescText">
          {gameState === "New"
            ? "Click the dice to hold a number, roll when all matching dice are held before the time runs out."
            : null}
        </div>
      </div>

      <div className="diceCont">{diceElements}</div>

      <div className="btnCont">
        <button className="main-rollBtn" onClick={rollDice}>
        ...
        {gameState === "Lost" && (
          <button
            ...
            onClick={() =>
              scoreDownloader(
                exportRef,
                `I Reached Level ${level} in Numberino`
              )
            }
          >
            Share Score
